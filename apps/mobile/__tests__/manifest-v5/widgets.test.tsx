...
import { TextWidget } from '@/manifest-v5/render/widgets/TextWidget';
import { StatWidget } from '@/manifest-v5/render/widgets/StatWidget';
import { ToggleWidget } from '@/manifest-v5/render/widgets/ToggleWidget';
import { ButtonWidget } from '@/manifest-v5/render/widgets/ButtonWidget';
import { SliderWidget } from '@/manifest-v5/render/widgets/SliderWidget';
import { SelectWidget } from '@/manifest-v5/render/widgets/SelectWidget';
...
describe('premium manifest widgets', () => {
  it('renders text widgets as premium content cards', () => {
    ...
    expect(getByTestId('v5-text-widget')).toBeTruthy();
  ...
  it('renders destructive buttons with premium danger styling', () => {
    ...
    expect(StyleSheet.flatten(getByTestId('v5-button-widget').props.style).backgroundColor).toBe(
      palette.danger,
    );
  });
  ...
  it('disables action widgets when the bound action is unavailable', () => {
    ...
    expect(getByTestId('v5-button-widget').props.accessibilityState?.disabled).toBe(true);
  });
  ...
  it('renders a premium slider widget with current numeric value', () => {
    ...
    expect(getByTestId('v5-slider-widget')).toBeTruthy();
  ...
  it('renders a V5 select widget with available enum options', () => {
    ...
    expect(getByTestId('v5-select-widget')).toBeTruthy();
