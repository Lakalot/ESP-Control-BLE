...
import { triggerSelectionHaptic, triggerSoftImpactHaptic } from '../feedback/haptics';
...
/** Shape produced by useBleScan / useSppScan — the device list + scan controls. */
...
interface Props {
  scan: ScanController;
  /** Active transport — only affects a few copy strings (BLE vs SPP wording). */
  transport: 'ble' | 'spp';
}

/**
 * Presentational scan screen: hero header, stats, FlashList of DeviceCards and
 * the PIN prompt + navigation to the control screen. Driven entirely by the
 * injected ScanController, so the BLE and SPP scan screens reuse it unchanged.
 */
export function DeviceListView({ scan, transport }: Props) {
  ...
  const heroSubtitle = useMemo(() => {
    ...
    if (isScanning) {
      return isSpp
        ...
        : 'Recherche des appareils BLE disponibles.';
    }

    return 'Selectionnez un appareil pour ouvrir ses commandes.';
  }, [bleState, isScanning, isSpp]);

  const safeStartScan = () => {
    Promise.resolve(startScan()).catch((error) => console.error('[Scan] startScan error:', error));
  };
            ...
            {isScanning ? (
              <>
                ...
                <Text style={styles.emptyHint}>
                  {isSpp
                    ? 'Verifiez que votre ESP32 est alimente, proche et appairable en Bluetooth.'
              ...
              <>
                ...
                <Text style={styles.emptyHint}>
                  {isSpp
                    ? 'Lancez un scan pour rafraichir la liste des appareils Bluetooth a portee.'
