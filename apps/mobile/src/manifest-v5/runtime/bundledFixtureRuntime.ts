...
/**
 * Loads `assets/manifest_v5_demo.pb` from the Expo asset bundle and returns
 * a ready-to-use FixtureRuntime. Used by the pilot `controls` screen when
 * the manifestRuntime flag is 'v5'. Plan C will replace this with a real
 * BleRuntime factory.
 */
export async function loadBundledFixtureRuntime(): Promise<FixtureRuntime> {
  ...
  const module = require('../../../assets/manifest_v5_demo.pb') as number;
