...
void BleTransportV5::handleFrame(FrameKind kind, const uint8_t* body, size_t len) {
  switch (kind) {
    ...
    case FrameKind::Subscribe: {
      manifest_v5_Subscribe sub = manifest_v5_Subscribe_init_zero;
      pb_istream_t is = pb_istream_from_buffer(body, len);
      if (pb_decode(&is, manifest_v5_Subscribe_fields, &sub)) {
        for (size_t i = 0; i < sub.resource_ids_count; ++i) _subs.add(sub.resource_ids[i]);
      }
      return;
    }
    case FrameKind::Unsubscribe: {
      manifest_v5_Unsubscribe uns = manifest_v5_Unsubscribe_init_zero;
      pb_istream_t is = pb_istream_from_buffer(body, len);
      if (pb_decode(&is, manifest_v5_Unsubscribe_fields, &uns)) {
        for (size_t i = 0; i < uns.resource_ids_count; ++i) _subs.remove(uns.resource_ids[i]);
      }
      return;
    }
